...
from src.step6_cv.run_cv_finetune import (
    MODEL_NAME_DEFAULTS,
...
def resolve_experiment_names(baseline_cfg: dict[str, Any]) -> list[str]:
    ...
    if raw is None:
        return [resolve_experiment_name(load_step4_config())]
    if isinstance(raw, str):
        names = [part.strip() for part in raw.split(",")]
    else:
        names = [str(part).strip() for part in raw]
    names = [BASE_EXPERIMENT_NAME if name in {"", "."} else name for name in names if name]
    if not names:
        raise ValueError("step6_cv.local_slm_baseline.experiments is empty")
    return names
...
def resolve_enabled_models(baseline_cfg: dict[str, Any]) -> list[str]:
    ...
    if isinstance(raw, list):
        ...
        raise ValueError("step6_cv.local_slm_baseline.models must be a list or mapping")
    ...
    if not model_names:
        raise ValueError("No enabled models in step6_cv.local_slm_baseline.models")
    return model_names
...
def get_target_folds(baseline_cfg: dict[str, Any]) -> set[int] | None:
    ...
    if not folds:
        raise ValueError("step6_cv.local_slm_baseline.folds is empty")
    return folds
...
def run_model_baseline(
    ...
    try:
        for experiment_name in experiment_names:
            manifest_path = resolve_experiment_dir(sft_output_dir, experiment_name) / "manifest.json"
...
def main() -> None:
    ...
    if not baseline_cfg.get("enabled", False):
        logger.info("step6_cv.local_slm_baseline.enabled is false; nothing to run.")
        return
