...
# ### Data Prep
# 
# This project uses text-only SFT data. We keep Ministral 3B on the `FastVisionModel` training path used by Unsloth, but format each turn as typed text content without any image inputs.
# 
...
# Let's take an overview look at the text-only dataset and inspect one sample conversation.
# 
...
# For text-only training with `FastVisionModel`, format each turn as typed text content:
# 
# ```python
# [
...
# Let's inspect one converted sample after formatting:
...
# sample_messages = converted_dataset[1]["messages"][:-1]
# input_text = tokenizer.apply_chat_template(sample_messages, add_generation_prompt=True)
# inputs = tokenizer(
#   text=input_text,
#   add_special_tokens=False,
#   return_tensors="pt",
...
if False:
  ...
  model, tokenizer = FastVisionModel.from_pretrained(
    model_name="ministral_lora",
    load_in_4bit=True,
  )
  FastVisionModel.for_inference(model)

sample_messages = converted_dataset[0]["messages"][:-1]
input_text = tokenizer.apply_chat_template(sample_messages, add_generation_prompt=True)
inputs = tokenizer(
  text=input_text,
  add_special_tokens=False,
